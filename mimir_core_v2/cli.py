...
def run_scan(
    ...
    progress.emit(
        ...
        "Optional AI second-opinion review complete." if vlm else "Local review complete; experimental AI was not enabled.",
        ai_review_attempted_groups if vlm else len(event_groups),
        max(ai_review_attempted_groups, 1) if vlm else max(len(event_groups), 1),
        incidents_created=len(incidents),
    ...
    session["runtime_identity"] = {
        ...
        "progress_protocol": "mimir_progress_v1",
    }
    ...
    progress.emit("writing_results", "Writing the session and diagnostics.", 0, 1, incidents_created=len(incidents))
    output_path = write_latest_session(session, output)
    progress.emit("writing_results", "Session written.", 1, 1, incidents_created=len(incidents))
    progress.complete("Scan complete.", len(incidents))
